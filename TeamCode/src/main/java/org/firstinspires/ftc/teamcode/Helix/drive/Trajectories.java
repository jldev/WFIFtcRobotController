...
public class Trajectories {
   ...
   public int redBlue = 1; //  red = 1 | blue = -1
    ...
    public Trajectories(Helix helix){
        this.mDrive = helix.drive;
        this.mStartPosition = helix.startPos;
        this.helix = helix;


        if (this.helix.allianceColor == Helix.AllianceColor.BLUE){
            redBlue = -1;
        }
    ...
    public void setupTrajectories(PropPlacement propLocation){
        switch(propLocation){
            case LEFT:
                targettedAprilTag = 4;
                if (helix.fieldPos == Helix.FieldPos.BD)
...
*/
    ...
    public Trajectory getTrajectoryForAprilTag(AprilTagPoseFtc pose, double distanceFromTag){
        ...
        if(helix.allianceColor == Helix.AllianceColor.BLUE){
            traj = mDrive.trajectoryBuilder(mStartPosition, true, TRAJECTORY_SPEED_SLOW)
