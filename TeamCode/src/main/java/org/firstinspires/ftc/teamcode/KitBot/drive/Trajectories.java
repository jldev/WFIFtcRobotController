...
public class Trajectories {
   ...
   public int redBlue = 1; //  red = 1 | blue = -1
    ...
    public Trajectories(KitBot kitBot){
        this.mDrive = kitBot.drive;
        this.mStartPosition = kitBot.startPos;
        this.kitBot = kitBot;


        if (this.kitBot.allianceColor == KitBot.AllianceColor.BLUE){
            redBlue = -1;
        }
    ...
    public void setupTrajectories(PropPlacement propLocation){
        switch(propLocation){
            case LEFT:
                targettedAprilTag = 4;
                if (kitBot.fieldPos == KitBot.FieldPos.BD)
...
*/
    ...
    public Trajectory getTrajectoryForAprilTag(AprilTagPoseFtc pose, double distanceFromTag){
        ...
        if(kitBot.allianceColor == KitBot.AllianceColor.BLUE){
            traj = mDrive.trajectoryBuilder(mStartPosition, true, TRAJECTORY_SPEED_SLOW)
